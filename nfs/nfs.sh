...
while true ; do
let num+=1
yum -y install iotop iftop yum-utils nfs-utils rpcbind net-tools rsync git lrzsz expect gcc gcc-c++ make cmake libxml2-devel openssl-devel curl curl-devel unzip sudo ntp libaio-devel wget vim ncurses-devel autoconf automake zlib-devel  python-devel bash-completion
if [[ $? -eq 0 ]] ; then
echo "初始化安装环境配置完成！！！"
break;
else
...
deploy(){
cd $bash_path
test -d $nfs_path || mkdir -p $nfs_path
chmod 755 $nfs_path
