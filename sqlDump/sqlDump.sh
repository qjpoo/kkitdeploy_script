...
install_mysql(){
cd /etc/yum.repos.d/ && wget -i -c http://dev.mysql.com/get/mysql57-community-release-el7-10.noarch.rpm && yum -y install mysql57-community-release-el7-10.noarch.rpm && yum -y install mysql-community-server
}
...
chmod 600 $bash_path/mysqlBackup.sh
chmod +x $bash_path/mysqlBackup.sh
echo "$workTime $bash_path/mysqlBackup.sh" >> /var/spool/cron/root   

}

checkScript(){
`$bash_path/mysqlBackup.sh`
if [[ $? -eq 0 ]];then
echo "备份成功！！！"
else
echo "备份脚本似乎有问题，请登录查看"
fi
}
...
#  ulimit_config
  ...
  checkScript
  echo "请登录到master服务器，查看$backup_location下目录是否有备份文件，cat /etc/crontab是否已经将定时任务写入"
}
main
